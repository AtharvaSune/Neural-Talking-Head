...
from .components import ResidualBlock, AdaptiveResidualBlock, ResidualBlockDown, AdaptiveResidualBlockUp, SelfAttention, ResidualBlockUp
import config
...
class Generator(nn.Module):
    ...
    def forward(self, y, e):
        ...
        out = self.in11_d(self.deconv11(out)) # [B, 1, 512, 512]
        out = self.in12_d(self.deconv12(out)) # [B, 1, 1024, 1024]
...
class Discriminator(nn.Module):
    ...
    def forward(self, x, y, i):
        ...
        _W_i = (self.W[:, i].unsqueeze(0)).transpose(0, 1)
        out = torch.bmm(_out, _W_i + self.w_0.unsqueeze(0)) + self.b
        out = torch.sigmoid(out)
